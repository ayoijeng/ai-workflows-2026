...
def reasoning_node(state: DataState) -> DataState:
    """Use LLM to decide whether to clean missing values or remove outliers or do both."""
    prompt = (
        "You are a data science assistant. "
        "Given this dataset summary, decide which single action is most appropriate: "
        "'clean_missing', 'remove_outliers', or 'remove_outliers_and_clean_missing'.\n\n"
        f"{state['summary']}\n\n"
        "Respond with only one of the following options: clean_missing, remove_outliers, remove_outliers_and_clean_missing."
        "Check all 3 options carefully before making a decision."
    )
    decision = llm.invoke(prompt).content.strip().lower()
    if decision not in ["clean_missing", "remove_outliers", "remove_outliers_and_clean_missing"]:
        decision = "none"
...
def remove_outliers_and_clean_missing(state: DataState) -> DataState:
    """Clean both missing values and remove outliers."""
    df = state["df"].copy()
    numeric_cols = df.select_dtypes(include="number").columns
...
def route_action(state: DataState) -> str:
    ...
    mapping = {
        ...
        "remove_outliers_and_clean_missing": "remove_outliers_and_clean_missing",
        "none": "describe_data",
    }
...
workflow.add_node("remove_outliers_and_clean_missing", remove_outliers_and_clean_missing)
...
workflow.add_edge("remove_outliers_and_clean_missing", "describe_data")
