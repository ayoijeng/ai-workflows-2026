...
def get_agent_exec_globals() -> dict:
    """
    Build the namespace for executing LLM-generated cleaning code.

    Pre-imports common data-science libraries so generated code can use them
    without hitting ModuleNotFoundError when only pandas/numpy are referenced
...
def execute_agent_code(state, data_key, code_snippet_key, result_key, error_key, agent_function_name):
    ...
    local_vars = {}
    global_vars = get_agent_exec_globals()
